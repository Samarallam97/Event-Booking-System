...
public class MappingProfiles : Profile
{
    public  MappingProfiles()
    {
		...
		CreateMap<Event, EventDTOEn>().ForMember(des => des.CategoryName, opt => opt.MapFrom(
			...
			;
		CreateMap<Event, EventDTOAR>().ForMember(des => des.CategoryNameAR, opt => opt.MapFrom(
			src => src.Category.NameAR)).ForMember(des => des.EventTagsNamesAR, opt => opt.MapFrom(
			src => src.EventTags.Select(et => et.Tag.NameAR))); ;
